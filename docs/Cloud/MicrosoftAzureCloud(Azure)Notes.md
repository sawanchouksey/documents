# ☁️ Microsoft Azure Cloud Notes: The Architect's Handbook
Welcome to the comprehensive Azure Cloud operations and architecture guide. This document is categorized functionally for rapid referencing, exam preparation, and deep-dive technical operations.

---

## 📑 Table of Contents
- [📚 1. AZ-305 & Exam Architecture Notes](#1-az-305-exam-architecture-notes)
- [🔗 2. Quick References & Useful Links](#2-quick-references-useful-links)
- [☁️ 3. Compute & Container Operations](#3-compute-container-operations)
- [🔐 4. Security, Identity & Compliance](#4-security-identity-compliance)
- [🌐 5. Networking & Traffic Management](#5-networking-traffic-management)
- [💾 6. Storage, Databases & Analytics](#6-storage-databases-analytics)
- [🚀 7. Azure CLI, PowerShell & ARM Snippets](#7-azure-cli-powershell-arm-snippets)
- [🛠️ 8. Management, Monitoring & Automation](#8-management-monitoring-automation)
- [🧩 9. Miscellaneous Basics](#9-miscellaneous-basics)

---

## 📚 1. AZ-305 & Exam Architecture Notes
...
## 🔗 2. Quick References & Useful Links
...
```
azcopy make "https://<STORAGE_ACCOUNT>.blob.core.windows.net/tmp?sv=2020-08-04&ss=b&srt=sco&sp=rwdlac&se=2021-12-13T14:36:11Z&st=2021-12-13T06:36:11Z&spr=https&sig=<SAS_SIGNATURE>"
```

- To upload a file

```
azcopy copy storage1.arm.json "https://<STORAGE_ACCOUNT>.blob.core.windows.net/tmp/storage1.arm.json?sv=2020-08-04&ss=b&srt=sco&sp=rwdlac&se=2021-12-13T14:36:11Z&st=2021-12-13T06:36:11Z&spr=https&sig=<SAS_SIGNATURE>"
```

- To upload a directory

```
azcopy copy "newdir/*" "https://<STORAGE_ACCOUNT>.blob.core.windows.net/tmp?sv=2020-08-04&ss=b&srt=sco&sp=rwdlac&se=2021-12-13T14:36:11Z&st=2021-12-13T06:36:11Z&spr=https&sig=<SAS_SIGNATURE>"
```

- To upload a directory to a directory in the container

```
azcopy copy "newdir/*" "https://<STORAGE_ACCOUNT>.blob.core.windows.net/tmp/newdir?sv=2020-08-04&ss=b&srt=sco&sp=rwdlac&se=2021-12-13T14:36:11Z&st=2021-12-13T06:36:11Z&spr=https&sig=<SAS_SIGNATURE>"
```

- To upload a directory and subdirectories to a directory in the container

```
azcopy copy "newdir/*" "https://<STORAGE_ACCOUNT>.blob.core.windows.net/tmp/newdir?sv=2020-08-04&ss=b&srt=sco&sp=rwdlac&se=2021-12-13T14:36:11Z&st=2021-12-13T06:36:11Z&spr=https&sig=<SAS_SIGNATURE>" --recursive
```

- Download blob data

```
azcopy copy "https://<STORAGE_ACCOUNT>.blob.core.windows.net/tmp/storage1.arm.json?sv=2020-08-04&ss=b&srt=sco&sp=rwdlac&se=2021-12-13T14:36:11Z&st=2021-12-13T06:36:11Z&spr=https&sig=<SAS_SIGNATURE>" "storage1.arm.json"
```

- copy data between two storage accounts

```
azcopy copy "https://<STORAGE_ACCOUNT>.blob.core.windows.net/tmp?sv=2020-08-04&ss=b&srt=sco&sp=rwdlac&se=2021-12-13T14:36:11Z&st=2021-12-13T06:36:11Z&spr=https&sig=<SAS_SIGNATURE>" "https://<DEST_STORAGE_ACCOUNT>.blob.core.windows.net/tmp?sv=2020-02-10&ss=b&srt=sco&sp=rwlac&se=2021-04-12T22:26:24Z&st=2021-04-12T14:26:24Z&spr=https&sig=<SAS_SIGNATURE>" --recursive
```

- remove folder from command 

```
azcopy remove "http://<STORAGE_ACCOUNT>.file.core.windows.net/<FILE_SHARE_NAME>/user/poc-5678-home/logs/os-pos-services?sv=2020-10-02&se=2022-03-11T05%3A40%3A29Z&sr=s&sp=rwdl&sig=<SAS_SIGNATURE>" --recursive --log-level=INFO;
./azcopy.exe remove "http://<STORAGE_ACCOUNT>.file.core.windows.net/<FILE_SHARE_NAME>/user/poc-5678-home/logs/os-pos-services?sv=2020-10-02&se=2022-03-11T05%3A40%3A29Z&sr=s&sp=rwdl&sig=<SAS_SIGNATURE>" --from-to=FileTrash --recursive --log-level=INFO;
$env:AZCOPY_CRED_TYPE = "Anonymous";
./azcopy.exe remove "https://<STORAGE_ACCOUNT>.file.core.windows.net/<FILE_SHARE_NAME>/microservices-new/logs/os-transaction-poster-application?sv=2021-04-10&se=2022-10-26T10%3A05%3A19Z&sr=s&sp=rwdl&sig=<SAS_SIGNATURE>" --from-to=FileTrash --recursive --log-level=INFO;
...
azcopy copy "ga.sql" "http://<STORAGE_ACCOUNT>.file.core.windows.net/<FILE_SHARE_NAME>?sv=2020-08-04&ss=bfqt&srt=sco&sp=rwdlacupitfx&se=2021-11-26T23:05:25Z&st=2021-11-26T15:05:25Z&spr=http&sig=<SAS_SIGNATURE>"
```
...
azcopy copy "http://<STORAGE_ACCOUNT>.file.core.windows.net/<FILE_SHARE>/<FILE_NAME>?<SAS_TOKEN>" "<LOCAL_PATH>"
```

- upload contents,file,directory in blob storage

```
az storage blob upload-batch -s <LOCAL_SOURCE_PATH> -d $web --connection-string=DefaultEndpointsProtocol=http;AccountName=<ACCOUNT_NAME>;AccountKey=<YOUR_ACCOUNT_KEY>;EndpointSuffix=core.windows.net
az storage blob upload-batch -s <LOCAL_SOURCE_PATH> -d <CONTAINER_NAME> --connection-string=DefaultEndpointsProtocol=http;AccountName=<ACCOUNT_NAME>;AccountKey=<YOUR_ACCOUNT_KEY>;EndpointSuffix=core.windows.net
```

- delete content from blob storage

```
az storage blob delete-batch -s $web\<CONTAINER_NAME> --connection-string=DefaultEndpointsProtocol=http;AccountName=<ACCOUNT_NAME>;AccountKey=<YOUR_ACCOUNT_KEY>;EndpointSuffix=core.windows.net 
```

---

## ☁️ 3. Compute & Container Operations
#### Availablity Set VMs
...
## 🚀 7. Azure CLI, PowerShell & ARM Snippets
...
```
az storage blob upload-batch -s <LOCAL_SOURCE_PATH> -d $web --connection-string=DefaultEndpointsProtocol=https;AccountName=<ACCOUNT_NAME>;AccountKey=<YOUR_ACCOUNT_KEY>;EndpointSuffix=core.windows.net
```

#### delete content from blob storage

```
az storage blob delete-batch -s $web --connection-string=DefaultEndpointsProtocol=https;AccountName=<ACCOUNT_NAME>;AccountKey=<YOUR_ACCOUNT_KEY>;EndpointSuffix=core.windows.net 
```
...
```
az storage blob upload-batch -s <LOCAL_SOURCE_PATH> -d <CONTAINER_NAME> --connection-string=DefaultEndpointsProtocol=https;AccountName=<ACCOUNT_NAME>;AccountKey=<YOUR_ACCOUNT_KEY>;EndpointSuffix=core.windows.net
```
...
2. create string pass all certificates i.e.
   ...
   ```
   String url =
   "jdbc:postgresql://<DB_HOST>:5432/<DB_NAME>?sslmode=prefer&sslrootcert=<PATH>/server-ca.pem&sslcert=<PATH>/client-cert.pem&sslkey=<PATH>/client-key.pk8&sslpassword=<YOUR_SSL_PASSWORD>";
   ```
...
## 🛠️ 8. Management, Monitoring & Automation
